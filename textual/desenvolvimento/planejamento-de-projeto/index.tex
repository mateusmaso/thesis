...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[font=\small]
    ...
    \node[rectangle,draw,minimum height=1cm,below of=client2,node distance=2.3cm] (tool) {$Tool$};
...
\begin{figure}[H]
  \centering
  \smartdiagramset{circular distance=2cm,uniform arrow color=true,arrow line width=1pt,arrow color=gray!50!black,uniform color list=white for 6 items,font=\small,text width=2.0cm,module minimum width=2.0cm,module minimum height=1.5cm,arrow tip=to,back arrow disabled=true}
  \smartdiagram[flow diagram:horizontal]{(1) Construir esquema para serviço,(2) Envolver campos do esquema, (3) Unificar esquemas}
  
%   \begin{tikzpicture}[font=\small,text width=5em, text centered]
%     \node [rectangle,draw,minimum height=4em,node distance=3cm] (buildSchema) {(1) Construir esquema para serviço};
%     \node [ellipse,draw,draw, above of=buildSchema,node distance=2cm] (services) {Serviços};
%     \node [rectangle,draw,minimum height=4em, right of=buildSchema,node distance=3cm] (wrapSchema) {(2) Envolver campos do esquema};
%     \node [rectangle,draw,minimum height=4em, right of=wrapSchema,node distance=3cm] (deepExtendSchema) {(3) Unificar esquemas};
...
%     \draw [->,dashed] (services) -- (buildSchema);
...
\begin{figure}[H]
  \centering
  \smartdiagramset{font=\small,circular distance=2cm,uniform arrow color=true,arrow line width=1pt,arrow color=gray!50!black,uniform color list=white for 7 items,font=\small,text width=2.0cm,module minimum width=2.0cm,module minimum height=1.5cm,arrow tip=to,back arrow disabled=true}
  \smartdiagram[flow diagram:horizontal]{(4) Simplificar AST,(5) Transformar AST,(6) Reduzir ASTs,(7) Desfazer envoltório de AST,(8) Buscar de dados,(9) Criar envoltório,(10) Unificar dados}
%   \begin{tikzpicture}[font=\small,text width=5em, text centered]
%     \node [rectangle,draw,minimum height=4em] (simplifyAST) {(4) Simplificar AST};
%     \node [ellipse,draw, above of=simplifyAST,node distance=2cm] (ast) {AST};
%     \node [rectangle,draw,minimum height=4em, below of=simplifyAST,node distance=2cm] (transformAST) {(5) Transformar AST};
%     \node [rectangle,draw,minimum height=4em, right of=transformAST,node distance=3cm] (reduceASTs) {(6) Reduzir ASTs};
%     \node [rectangle,draw,minimum height=4em, right of=reduceASTs,node distance=3cm] (unwrapAST) {(7) Desfazer envoltório de AST};
%     \node [rectangle,draw,minimum height=4em, right of=unwrapAST,node distance=3cm] (fetchData) {(8) Buscar de dados};
%     \node [rectangle,draw,minimum height=4em, above of=fetchData,node distance=2cm] (wrapData) {(9) Criar envoltório};
%     \node [rectangle,draw,minimum height=4em, above of=wrapData,node distance=2cm] (deepExtendData) {(10) Unificar dados};
%     \node [ellipse,draw, above of=deepExtendData,node distance=2cm] (data) {Dados};
...
%     \draw [->] (wrapData) -- (deepExtendData);
%     \draw [->,dashed] (ast) -- (simplifyAST);
%     \draw [->,dashed] (deepExtendData) -- (data);
%   \end{tikzpicture}
  \caption{Consulta de dados no intermediador (resolveQuery)}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[font=\small]
    ...
    \draw [->,loop above] (ast) -- (ast) node [pos=0.5,above,font=\footnotesize] {$4$};
    \draw [->] (ast) -- (astTransformed1) node [pos=0.5,above,font=\footnotesize] {$5$};
    \draw [->] (ast) -- (astTransformedN) node [pos=0.5,above,font=\footnotesize] {$5$};
