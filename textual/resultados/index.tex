...
Apesar dos ganhos de performance e desenvolvimento através do uso da ferramenta no cliente 2. Percebe-se na figura 32 um outro cenário, em que mostra-se o lado negativo da ferramenta através do tempo de \textit{overhead}\footnote{
  Processamento em excesso
} levado para que seja feito as buscas. Em média, a ferramenta atrasou em 1.7 segundos a busca de dados, sendo o tempo de busca de metadados responsável por somar mais da metade deste atraso inicial.

Contudo, é importante levar em conta que esse tempo de \textit{overhead} é um trade-off (impasse) e relativo ao tempo de vida no qual o cliente é executado pelo usuário. Para clientes com tempo de vida curto ou que dependem de \textit{bootstrap}\footnote{

} rápido, a ferramenta pode não ser a solução ideal. Vale lembrar que este tempo é também relativo pois depende da implementação dos algoritmos da ferramenta.
